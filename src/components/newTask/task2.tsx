...
const Task2 = props => {
  ...
  const { task, currentPos, wrongInput, currentGameCharacter } = props;

  const inputElement = useRef<HTMLDivElement | null>(null);
  const audioElement: React.MutableRefObject<HTMLMediaElement | null> = useRef<HTMLMediaElement | null>(
    null
  );

  /*  useEffect(() => {
     props.setTask(task2value);
   }, [props]); */
  ...
  useEffect(() => {
    ...
    speak(task.exercise[currentPos].text, ttsOptionsInEffect)
      .then(url =>
        playAudio(audioElement, url).catch(error =>
          console.error("play error intial character " + error)
        )
      )
      .catch(error => console.error("speak inital character errror " + error));
  ...
  const handleKey = (event: React.KeyboardEvent): void => {
    if (event.which !== 0 && audioElement.current) {
      ...
      const correctKeyPressed =
        event.key.toLowerCase() === task.exercise[currentPos].text;
      ...
      if (correctKeyPressed) {
        ...
        playAudio(audioElement, assetBaseUrl + "correct.mp3")
          .then(() => {
            if (currentPos < task.exercise.length - 1) {
              speak(task.exercise[currentPos + 1].text, ttsOptions)
                .then(textURL => {
                  if (textURL !== "" && audioElement.current) {
                    playAudio(audioElement, textURL).catch(error =>
        ...
        playAudio(audioElement, assetBaseUrl + "wrongsound.mp3")
          .then(() => {
            const ttsOptionsSlow = { language: i18n.language, rate: 1 };
            speak(
              fingerPlacement(task.exercise[currentPos].text, i18n.language),
              ttsOptionsSlow
            )
              .then(textURL => {
                if (textURL !== "" && audioElement.current) {
        ...
        <React.StrictMode>
          ...
          >
            <Grid item container justify='center' spacing={2}>
              ...
              <Grid item xs={2}>
                <Typography variant='h2' className={classes.remainingText}>
                  {(currentPos + 1 < task.exercise.length
                    ? task.exercise[currentPos + 1].text
                    : "") +
                    (currentPos + 2 < task.exercise.length
                      ? task.exercise[currentPos + 2].text
                      : "")}
                </Typography>
              </Grid>
            </Grid>
            <Grid item xs={2}>
              <img
                ...
                alt={currentGameCharacter.name + " " + t("task.character")}
              />
            </Grid>
          </Grid>
          ...
          <Typography variant='body1'>
            {t("task.missionAlreadyCompleted")}
          </Typography>
        )}
    </Grid>
  );
